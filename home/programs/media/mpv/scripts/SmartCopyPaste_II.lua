...
function multipaste()
  ...
  if filePath ~= nil then
    for i = 1, #clip_table do
      ...
      elseif
        not has_value(o.paste_extensions, clip_table[i][3]) and not has_value(o.paste_subtitles, clip_table[i][3])
      then
  ...
  if #clip_table > 1 then
    ...
    if filePath == nil then
      ...
      elseif o.running_paste_behavior == "playlist" then
        if
          filePath ~= clip_file and file_exists(clip_file) and has_value(o.paste_extensions, currentVideoExtension)
